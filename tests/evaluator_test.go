...
func TestEvalArrayExpression(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(result.Elements) != tt.length {
			t.Errorf("array has wrong number of elements. got=%d, want=%d", len(result.Elements), tt.length)
		}

		if result.Inspect() != tt.expected {
			t.Errorf("Array is not correct. got=%q, want=%q", result.Inspect(), tt.expected)
		}
	}
}
...
func TestReturnStatementsEval(t *testing.T) {
	tests := []struct {
		input    string
		expected int64
	}{
